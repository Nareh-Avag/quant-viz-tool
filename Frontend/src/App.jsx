...
function App() {
  ...
  const handleOptimize = async () => {
    setLoading(true);
    try {
      // Connects to your Python backend (port 8000)
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-8 font-sans">
      ...
      <header className="mb-10 flex flex-col md:flex-row justify-between items-start md:items-center border-b border-slate-800 pb-8 gap-6">
        <div>
          ...
          <p className="text-slate-500 mt-1 uppercase tracking-widest text-xs font-bold">Portfolio Optimization Engine</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-slate-900/40 border border-slate-800/60 p-8 rounded-3xl backdrop-blur-xl">
          <div className="flex justify-between items-center mb-8">
            ...
            <div className="flex gap-4 text-xs font-mono">
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-slate-700"></span> Random Portfolios</span>
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-emerald-400"></span> Max Sharpe</span>
            </div>
          </div>
            ...
            {data ? (
              <ResponsiveContainer width="100%" height="100%">
                <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                  <XAxis type="number" dataKey="x" name="Volatility" unit="%" stroke="#475569" fontSize={12} tickLine={false} axisLine={false} />
                  <YAxis type="number" dataKey="y" name="Return" unit="%" stroke="#475569" fontSize={12} tickLine={false} axisLine={false} />
                  <Tooltip 
                    contentStyle={{ backgroundColor: '#0f172a', borderRadius: '12px', border: '1px solid #1e293b' }}
                    cursor={{ strokeDasharray: '3 3' }} 
                  />
                  <Scatter data={data.scatter} fill="#334155" opacity={0.4} shape="circle" />
                  <Scatter data={[{x: data.optimal.risk, y: data.optimal.return}]} fill="#34d399">
              ...
              <div className="flex flex-col items-center justify-center h-full text-slate-600 border-2 border-dashed border-slate-800 rounded-2xl">
                ...
                <p className="font-medium uppercase tracking-tighter">Enter assets and run engine to visualize risk/return</p>
              </div>
            )}
          </div>
        </div>

        {/* SIDE PANEL: WEIGHTS & ANALYTICS */}
        <div className="space-y-8">
          <div className="bg-slate-900/40 border border-slate-800/60 p-8 rounded-3xl">
            ...
            {data ? (
              <div className="space-y-5">
                 {Object.entries(data.optimal.weights).map(([ticker, weight]) => (
                   <div key={ticker}>
                     ...
                     <div className="w-full bg-slate-800 rounded-full h-1.5">
                       <div className="bg-emerald-500 h-1.5 rounded-full" style={{ width: `${weight * 100}%` }}></div>
                     </div>
                   </div>
                 ))}
              </div>
          ...
          <div className="bg-gradient-to-br from-blue-600 to-indigo-700 p-8 rounded-3xl shadow-xl shadow-blue-900/20">
            ...
            <p className="text-blue-100 text-xs mb-6">Annualized projections based on historical 1Y window</p>
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-white/10 p-4 rounded-xl backdrop-blur-md">
                <p className="text-blue-200 text-[10px] uppercase font-bold">Sharpe Ratio</p>
                <p className="text-2xl font-black text-white">{data ? data.optimal.sharpe.toFixed(2) : "0.00"}</p>
              </div>
              <div className="bg-white/10 p-4 rounded-xl backdrop-blur-md">
                <p className="text-blue-200 text-[10px] uppercase font-bold">Volatility</p>
                <p className="text-2xl font-black text-white">{data ? data.optimal.risk.toFixed(1) : "0.0"}%</p>
              </div>
              <div className="bg-white/10 p-4 rounded-xl backdrop-blur-md col-span-2">
                ...
                <p className="text-2xl font-black text-white">{data ? data.optimal.return.toFixed(1) : "0.0"}%</p>
              </div>
            </div>
          </div>
        </div>
